...
def _wrap_models_in_pipeline(
    ...
    for model in models:
        ...
        # Add target PCA transformer if specified
        if "pca" in preprocess_outputs:
            steps.append(
                ("PCA applied to target values", TargetPCA(n_components=3))
            )  # Adjust n_components as needed
        # Add VAE transformer for Y if specified
        if "vae" in preprocess_outputs:
            print(model)
            steps.append(
                ("VAE applied to target values", VAEOutputPreprocessor(latent_dim=3))
            )  # No need for input_dim here
